#데이터불러오기 ####
setwd("C:/Users/82104/Desktop/제출용")
...
result <- rbind(auc_P,auc_C,f1score_P,f1score_C)
result 
#auc_P = 일정심각도 정확도, auc_C = 금액심각도 정확도, f1score_P = 일정심각도 f1-score, f1score_C = 금액심각도 f1-score

#모델 저장
#save("model_NBP",file="C:/Users/82104/Desktop/상아매니지먼트/Period_model.RData")
#save("model_RFC",file="C:/Users/82104/Desktop/상아매니지먼트/Cost_model.RData")

# 십분위 향상차트
period_pred <- predict(model_NBP, x.test, type="class")
cost_pred <- predict(model_RFC, x.test, type='prob')[,2]
period_pred = ifelse(period_pred == "심각",1,0)

y.test.period <- newdata[-train,98] #검증데이터용 일정심각도
y.test.cost <- newdata[-train,99] #검증데이터 금액심각도
y.test.cost <- as.factor(y.test.cost)
y.test.period <- as.factor(y.test.period)
y.test.cost = ifelse(y.test.cost == "심각",1,0)
y.test.period = ifelse(y.test.period == "심각",1,0)
...
lift_df = data.frame(period_pred, cost_pred, y.test.cost, y.test.period)
#tbl_df(lift_df)           

#lift.cost_df <- caret::lift(y.test.cost~cost_pred, cuts=11, class=1)
#lift.period_df <- caret::lift(y.test.period~period_pred, cuts=11, class=1)

#gains_tbl <- gains(actual = as.integer(lift_df$y.test.cost),
#                   predict = lift_df$cost_pred,
#                   groups=8)

gains_tbl <- gains(as.integer(lift_df$y.test.cost),predict = lift_df$cost_pred)
barplot(gains_tbl$mean.resp/mean(as.integer(lift_df$y.test.cost)), names.arg=gains_tbl$depth)
